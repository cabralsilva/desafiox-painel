...
const defaultQualifiedRule: QualifiedRule = {
  priority: 1,
  mode: "POSITION_GENERAL",
  position: 1,
  limit: 1,
};

function defaultPhase(order: number): ChampionshipPhase {
  return {
    description: "",
    order,
    type: "PONTOS-CORRIDOS",
    amountSlotsByGroup: undefined,
    groups: [{ name: "Grupo", slots: [] }],
    matchRules: { mode: "SAME_GROUP", twoLegs: false },
...
export default function ChampionshipForm() {
  ...
  const [modality, setModality] = useState<Team["modality"]>("SOCCER");
  const [teamIds, setTeamIds] = useState<string[]>([]);
  const [teams, setTeams] = useState<Team[]>([]);
  const [phases, setPhases] = useState<ChampionshipPhase[]>([defaultPhase(0)]);
  ...
  useEffect(() => {
    ...
    Promise.all([getChampionshipById(id), getChampionshipPhases(id)])
      .then(([champ, phaseList]) => {
        ...
        setTeamIds(champ.teamIds);
        const mapped = phasesToForm(phaseList);
  ...
  const setPhase = useCallback((index: number, patch: Partial<ChampionshipPhase>) => {
  ...
  const validatePhase = (p: ChampionshipPhase) => {
    const next: Record<string, string> = {};
      ...
      <form onSubmit={step === 1 ? handleNext : handleSubmitAll} className="space-y-5 p-4 md:p-6">
          ...
          <>
            ...
            <Field label="Modalidade">
              <select
                value={modality}
                onChange={(e) => {
                  setModality(e.target.value as Team["modality"]);
                  setTeamIds([]);
                }}
            ...
            <div>
              <div className="mb-2 flex items-center justify-between gap-2">
                ...
                <div className="flex gap-3 text-sm font-semibold">
                  <button type="button" className="text-accent" onClick={() => setTeamIds(teamsFiltered.map((t) => t.id))}>
                    Todos
                  </button>
                ...
                {teamsFiltered.map((t) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex min-h-12 items-center gap-3 rounded-xl px-3 text-left ${
                        checked ? "bg-primary/20 ring-1 ring-primary" : "bg-secondary"
                      }`}
                    >
                      <span
        ...
        {step >= 2 && currentPhase && (
          <PhaseFields
            ...
            teams={teamsFiltered.filter((t) => teamIds.includes(t.id))}
...
function PhaseFields({
  ...
  onChange: (patch: Partial<ChampionshipPhase>) => void;
  ...
  const applySlots = (amount: number, groups = phase.groups) => {
    const nextGroups = groups.map((g) => {
      const slots = [...g.slots];
      while (slots.length < amount) slots.push({ label: `Posição ${slots.length + 1}`, team: null });
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
        ...
        <select
          ...
          className="flex h-12 w-full rounded-md border border-input bg-card px-3 text-base"
        >
          {(Object.keys(GROUP_MATCH_MODE_LABELS) as GroupMatchMode[]).map((m) => (
            <option key={m} value={m}>
              {GROUP_MATCH_MODE_LABELS[m]}
            </option>
          ))}
        </select>
          ...
          <div key={gi} className="rounded-2xl border border-border bg-card p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
              {group.slots.map((slot, si) => {
                const team = teams.find((t) => t.id === slot.team);
                      ...
                      <div className="mt-2 max-h-40 overflow-y-auto rounded-xl border border-border p-2">
                        ...
                        {teams.map((t) => (
                          <button
                            ...
                            onClick={() => setSlotTeam(gi, si, t.id)}
                          >
                            {t.name}
                          </button>
                        ))}
        ...
        {phase.qualifiedRules.map((rule, ri) => (
          <div key={ri} className="grid grid-cols-2 gap-2 rounded-xl bg-secondary p-3 md:grid-cols-4">
            <Field label="Modo">
              ...
              >
                {(Object.keys(QUALIFICATION_MODE_LABELS) as QualificationMode[]).map((m) => (
                  <option key={m} value={m}>
                    {QUALIFICATION_MODE_LABELS[m]}
                  </option>
                ))}
              </select>
            </Field>
